...
public class AndroidNDKHelper {
		...
		if (json != null)
		{
			try
			{
				...
				if (obj.has(__CALLED_METHOD__))
				{
					...
					Log.e("---callmethod---",methodName);
					String methodParams = null;

					try
					{
						methodParams = obj.getString(__CALLED_METHOD_PARAMS__);
						Log.e("---methodParams---", String.valueOf(methodParams));
					}
					...
					try
					{
						...
						Method m = AndroidNDKHelper.callHandler.getClass().getMethod(methodName, new Class[] { String.class });
	...
	public static boolean SendMessageGetAdFlag(String methodName){
		final String methodName_ad = methodName;
		Log.e("----methodName--",methodName);
//		MAdsManager.getInstance().runOnMainThread(new Runnable() {
//			@Override
//			public void run() {
				try {
					Method m = AndroidNDKHelper.callHandler.getClass().getMethod(methodName_ad);
					try {
						isReturn = (Boolean) m.invoke(AndroidNDKHelper.callHandler);
					} catch (IllegalAccessException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					} catch (IllegalArgumentException e) {
...
//			}
//		});
		Log.e("----isReturn--", String.valueOf(isReturn));
		return isReturn;
